<?php
    ...
    abstract class AbstractContainer extends AbstractElement
    {
        ...
        protected function addElement($elementName)
        {
            ...
            // Fix list counting problem
            if (in_array($elementName, ['ListItem', 'ListItemRun'])) {
                $elementArgs = $this->fixListItemArguments($elementName, $elementArgs);
            }
        ...
        /**
         * Fix ListItem|ListItemRun arguments and set an initial numId as first argument, to separate lists
         * @author eweso
         * @param string $elementName
        ...
        protected function fixListItemArguments($elementName, array $elementArgs)
        {
            $prevElement = end($this->elements);

            if (is_object($prevElement)) {
                $prevElementName   = basename(str_replace("\\", '/', get_class($prevElement)));
                $parentElementName = basename(str_replace("\\", '/', get_class($this)));

                if (!in_array($prevElementName, ['ListItem', 'ListItemRun']) && in_array($parentElementName, ['ListItem', 'ListItemRun'])) {
                    $prevElementName = $parentElementName;
                    $prevElement     = $this;
                }

                // If prev Container or it's parent Container is not an ListItem or ListItemRun, then set new numId
                if ($prevElementName !== $elementName) {
                    if (isset($elementArgs[1])) {
                        $this->lastNumId = $this->countElements() + 1;
                        $args = [
                            'numId' => $this->lastNumId
                        ];
                    }
                    // If prev Container or it's parent Container ListItem or ListItemRun, then use last numId
                } else {
                    $args = [
                        'numId' => $this->lastNumId
                    ];
                ...
                foreach ($elementArgs[1] as $key => $value) {
                    $args[$key] = $value;
                }
